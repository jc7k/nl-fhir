...
# Custom Claude Code statusline - Created: 2025-09-08T23:07:07.256Z
...
# Get token data and session info from ccusage if available, or fallback to session file timestamps
...
# Fallback session time calculation using session file timestamps if ccusage not available
if [ -z "$session_txt" ] && [ -n "$session_file" ] && [ -f "$session_file" ] && [ "$HAS_JQ" -eq 1 ]; then
  # Get first and last timestamps from message entries in session file
  first_time=$(grep -m1 '"type":"message"' "$session_file" | jq -r '.timestamp // empty' 2>/dev/null)
  last_time=$(tail -20 "$session_file" | grep '"type":"message"' | tail -1 | jq -r '.timestamp // empty' 2>/dev/null)
  
  if [ -n "$first_time" ] && [ -n "$last_time" ]; then
    start_sec=$(to_epoch "$first_time")
    last_sec=$(to_epoch "$last_time")
    now_sec=$(date +%s)
    
    if [ -n "$start_sec" ] && [ -n "$last_sec" ] && [ "$start_sec" -gt 0 ] && [ "$last_sec" -gt 0 ]; then
      # Calculate session duration
      session_duration=$(( now_sec - start_sec ))
      hours=$(( session_duration / 3600 ))
      minutes=$(( (session_duration % 3600) / 60 ))
      
      # Simple session info without reset time (since we don't have usage limits info)
      session_txt="$(printf 'Session: %dh %dm active' "$hours" "$minutes")"
      
      # Show progress based on activity (last message vs start)
      if [ "$last_sec" -gt "$start_sec" ]; then
        activity_pct=$(( (last_sec - start_sec) * 100 / (now_sec - start_sec) ))
        # Cap at 100%
        [ "$activity_pct" -gt 100 ] && activity_pct=100
        session_bar=$(progress_bar "$activity_pct" 10)
      fi
    fi
  fi
fi
