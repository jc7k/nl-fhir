...
class ConsentFactory(BaseResourceFactory):
    ...
    def _add_metadata(self, resource: Dict[str, Any], request_id: Optional[str] = None):
        """
        Add FHIR R4-compliant metadata (override base class method).

        The base class adds non-compliant fields ('factory', 'created_at', 'version')
        which fail HAPI validation. This override uses FHIR-compliant meta fields.
        ...
        # Use FHIR-compliant tag for factory provenance
        if 'tag' not in resource['meta']:
            resource['meta']['tag'] = []

        resource['meta']['tag'].append({
            'system': 'http://nl-fhir.io/factory',
            'code': self.__class__.__name__,
            'display': f'Generated by {self.__class__.__name__}'
        })

        # Add request ID as identifier if provided
        if request_id:
            resource['meta']['tag'].append({
                'system': 'http://nl-fhir.io/request-id',
                'code': request_id
            })

        # Ensure resource has an ID (inherited from base)
