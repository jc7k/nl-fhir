...
## Status
**Completed** - Integrated into unified NLP pipeline with structured clinical output generation

## Story
**As a** clinical NLP system processing complex medical orders,  
**I want** to integrate PydanticAI (or Instructor + OpenAI Structured Outputs) for schema-constrained slot filling and structured output generation,  
**so that** complex clinical text can be processed into well-structured, validated medical data that complements entity extraction and RAG enhancement for comprehensive FHIR resource preparation.

## Acceptance Criteria

1. **LLM Integration**: Implement PydanticAI or Instructor + OpenAI integration with structured output schemas for medical data
2. **Schema-Constrained Processing**: Create Pydantic schemas that enforce medical data structure and validation rules
3. **Clinical Slot Filling**: Extract and fill structured medical information slots (dosage schedules, clinical context, treatment plans)
4. **NLP Pipeline Integration**: Seamlessly integrate structured LLM processing with existing spaCy/medspaCy (Story 2.1) and RAG (Story 2.2) components
5. **Medical Validation**: Implement medical logic validation for structured outputs (dose calculations, frequency validation, contraindication checks)
6. **Error Handling & Fallback**: Handle LLM processing failures with graceful degradation and fallback to rule-based extraction
7. **Performance Optimization**: Maintain <2s total response time while adding LLM processing to the pipeline
